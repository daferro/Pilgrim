...
def anarc4(T,A,B,n,T0,Tr=TR_DEFAULT):
    ...
    # Calculate rate constant
    k = A * ((T/Tr)**n) * np.exp(-B*(T+T0)/((T**2)+(T0**2)))
    # Return k
    return k
#====================#
...
def fit2anarc(tlist,rclist,log=True):
    ...
    GUESSES[2] = [A,B,0.0]
    GUESSES[3] = [A,B,0.0]
    GUESSES[4] = [A,B,0.0,298.15]
    # log is True or False??
    if log:
       FNCS   = LOGANFNC
       rclist = [np.log(k) for k in rclist]
    else:
       FNCS   = ANFNC
    #-----------------------#
    # Fitting and r^2       #
    ...
    for atype in sorted(FNCS.keys()):
        ...
        # fitting
        try   : popt, pcov  = curve_fit(function, tlist, rclist, p0=guess, maxfev=5000)
        except: continue
        # calculate r^2
        if len(popt) != nparams: continue
        r2 = get_r2(tlist,rclist,function,popt)
        # add Tr to return
        if atype in [3,4]: popt = list(popt)+[TR_DEFAULT]
        # save data
        dfit[atype] = (popt,r2)
    return dfit
#====================#
