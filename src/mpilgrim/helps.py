...
'''
*-----------------------------------*
| Module name:  mpilgrim.helps      |
| Last Update:  2019/02/26 (Y/M/D)  |
...
MENU  = '''
 ...
 Information about variables ($var):
   ...
   -----------------------------------------------------------------
    $var    | affects the input file| which contains...
   -----------------------------------------------------------------
    struc   | pif.struc             | the structures & isot. masses
    isomass | pif.struc             |          "
    temp    | pif.temp              | the temperatures
    chem    | pif.chem              | the reactions
    path    | pif.path & pif.calcs  | the minimum energy paths
    kmc     | pif.kmc               | the variables in the KMC
    dlevel  | pif.dlevel            | the structures for high-level
   -----------------------------------------------------------------
...
* 'add' : Initializes the KMC variables
  ...
  This option lists the KMC variables ($kmcvar), 
  ...
  $kmcvar can be:
     ...
     pop(ctcsp)_0 | initial number of particles of the 'ctcsp' species
  ...
  RATE CONSTANTS SPECIFICATION (inside 'add'):
    ...
    Command line for the rate constants calculated by Pilgrim:
      ...
      * $chemtype is the type of rate constant.
         ...
         Valid values for the harmonic version are:
            tst    cvt    tstzct    cvtzct    tstsct    cvtsct 

         Valid values for the anharmonic version are:
            anhtst anhcvt anhtstzct anhcvtzct anhtstsct anhcvtsct 
