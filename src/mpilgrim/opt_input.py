...
'''
*-----------------------------------*
| Module name:  mpilgrim.opt_input  |
...
#--------------------------------------------------#
from   opt_gather         import ls_struc
#--------------------------------------------------#
from   diverse            import get_input_data
from   diverse            import status_check
from   diverse            import ffchecking
from   diverse            import calc_fwdir
#--------------------------------------------------#
from   itf                import get_templates
#--------------------------------------------------#
from   PathVars           import PathVars
#--------------------------------------------------#
from helps import MENU
from helps import HELP_input_temp    as HELP_temp
from helps import HELP_input_chem    as HELP_chem
from helps import HELP_input_path    as HELP_path
from helps import HELP_input_kmc     as HELP_kmc
from helps import HELP_input_dlevel  as HELP_dlevel
from helps import HELP_input_isomass as HELP_isomass
from helps import HELP_input_struc   as HELP_struc
#--------------------------------------------------#
...
def add_path(targets,dpath,dctc,dtesLL,lts,mod=False):
    ...
    for target in targets:
        if mod and (target in dpath.keys()): continue
        dpath[target] = PathVars("mep")
        # get fwdir (this may take some time...)
        try   : fwdir = calc_fwdir(target)
        except: continue
        dpath[target]._fwdir = fwdir
    # print path(s)
    ls_path(dpath,targets)
    # modify them
    while True:
          try:
            line = raw_input(ILINE2).strip()
...
def ls_kmc(tkmc):
    ...
    if tkmc is None:
        ...
        for species,ipop in sorted(ipops.items()):
            string += ("   pop(%%-%is)_0 = %%.3e"%l1)%(species,ipop)
    ...
    for line in (string).split("\n"): print IBLANK2+line
#---------------------------------------------------------------#
def add_kmc(val,tkmc,dchem):
    set_completer(2)
    # previous data?
    if tkmc is None: tkmc = {}, {}, 1000, 1.0/ML, "ps", []
    # expand tuple
    ipops, rcs, psteps, volume, timeunits, excess = tkmc
    ...
    while True:
        ...
        try:
           ...
           tkmc = (ipops,rcs,psteps,volume,timeunits,excess)
        except:
           continue
    return tkmc
#===============================================================#
...
def mod_struc(target,dctc,dimasses):
    ...
    while True:
          try:
            line = raw_input(ILINE2).strip()
            if line.split()[0] in END: break
            if "=" not in line: line = line.split()
            else              : line = [ string.strip() for string in line.split("=")]
            ...
            for target in targets:
                ...
                ls_ind_struc(dctc[target],False)
