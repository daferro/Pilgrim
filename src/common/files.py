...
from   physcons import ANGSTROM, AMU, KCALMOL, CM2H, KJMOL
from   physcons import METER, JOULE, KB
from   dicts    import dpt_s2m
#=============================================#
...
#===============================================#
# Function(s) to deal with: ICS file(s)         #
#===============================================#
def read_icfile(f_ics):
    from internal import merge_ics, string2ic
    lines = read_file(f_ics)
    ...
    for line in lines:
        ...
        if "*" in ic:
           unremovable = True
           ic = "".join([ch for ch in ic if ch != "*"])
           ic = ic.strip()
        else:
           unremovable = False
        case, atoms = string2ic(ic)
        if case == "st": ics_st.append(atoms)
        if case == "ab": ics_ab.append(atoms)
        if case == "lb": ics_lb.append(atoms)
        if case == "it": ics_it.append(atoms)
        if case == "pt": ics_pt.append(atoms)
        # any un-removable?
        if unremovable: list_inmut.append( (case,atoms) )
    # put all as a single list
    all_ics = merge_ics(ics_st,ics_ab,ics_lb,ics_it,ics_pt)
    return all_ics, list_inmut
#-----------------------------------------------#
def write_icfile(f_ics,all_ics,keep=[]):
    from internal import unmerge_ics, ic2string
    # all as lists (so comparison does not fail)
    all_ics = [list(ic) for ic in all_ics]
    keep    = [list(ic) for ic in keep   ]
    # unmerge
    ...
    string  = "# stretching [%i]\n"%len(ics_st)
    for atoms in ics_st:
        if atoms in keep_st: string += ic2string(("st",atoms))+"*\n"
        else               : string += ic2string(("st",atoms))+" \n"
    string += "\n"
    # add angular bendings
    string += "# angular bending [%i]\n"%len(ics_ab)
    for atoms in ics_ab:
        if atoms in keep_ab: string += ic2string(("ab",atoms))+"*\n"
        else               : string += ic2string(("ab",atoms))+" \n"
    string += "\n"
    # add linear bendings
    string += "# linear  bending [2x%i]\n"%len(ics_lb)
    for atoms in ics_lb:
        if atoms in keep_lb: string += ic2string(("lb",atoms))+"*\n"
        else               : string += ic2string(("lb",atoms))+" \n"
    string += "\n"
    # add improper torsions
    string += "# torsions (improper) [%i]\n"%len(ics_it)
    for atoms in ics_it:
        if atoms in keep_it: string += ic2string(("it",atoms))+"*\n"
        else               : string += ic2string(("it",atoms))+" \n"
    string += "\n"
    # add proper torsions
    string += "# torsions (proper) [%i]\n"%len(ics_pt)
    for atoms in ics_pt:
        if atoms in keep_pt: string += ic2string(("pt",atoms))+"*\n"
        else               : string += ic2string(("pt",atoms))+" \n"
    string += "\n"
    # write file
    write_file(f_ics,string)
#===============================================#
...
def read_gauout(gauout):
    ...
    key1, key2 = "\GINC-","\@"
    lines = fncs.extract_string(lines,key1,key2,fmatch=False)
    lines = "".join([line.strip() for line in lines.split()])
    lines = lines.split("\\\\")
...
def read_fchk(fchkfile):
    ...
    return xcc,atonums,ch,mtp,E,gcc,Fcc,masses, calclevel.strip()
#-----------------------------------------------#
def fchk2gts(fchk,gts,rotsigma=None,pgroup="C1"):
    xcc,atonums,ch,mtp,E,gcc,Fcc,masses,clevel = assb.read_fchk(fchk)
    if rotsigma is None:
       pgroup, rotsigma = get_pgs(atonums,masses,xcc)
    assb.write_gtsfile(xcc,atonums,ch,mtp,E,pgroup,rotsigma,gcc,Fcc,gts,level=clevel)
#===============================================#
...
if __name__ == '__main__':
    from fncs import print_funcs_in_module
    print_funcs_in_module()
